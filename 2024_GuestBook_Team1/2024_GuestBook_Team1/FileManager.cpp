 ...
 *
 * 
 * @todo UI업데이트에 맞게 좌표 및 기능 재배치, 저장 및 불러오기에서 데이터 저장 기능이 안되는 오류 해결 
 */
...
void FileManager::FileManager_Open()
{
    ...
    InvalidateRect(hWnd, NULL, FALSE);  /// 창 갱신
}
...
void FileManager::SaveFile(HWND hWnd, std::vector<PINFO>* penMemory) {
    ...
    if (GetSaveFileName(&OFN)) {
        std::fstream outFile(file_name, std::ios::out);  // 파일 열기
        ...
        MessageBox(hWnd, L"파일 저장이 취소되었습니다.", L"오류", MB_OK | MB_ICONWARNING);
    }
}
...
void FileManager::SaveFileList() {
    std::wofstream outFile(L"C:\\Users\\siuugil\\Test\\2024_GuestBook_Team1\\savefile\\fileList.txt");  // 파일 리스트 저장
    if (outFile.is_open()) {
        for (const auto& file : fileList) {
...
void FileManager::LoadFileList() {
    std::wifstream inFile(L"C:\\Users\\siuugil\\Test\\2024_GuestBook_Team1\\savefile\\fileList.txt");  // 파일 리스트 불러오기
    if (inFile.is_open()) {
        std::wstring line;
        while (std::getline(inFile, line)) {
